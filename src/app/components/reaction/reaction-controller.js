...
class ReactionController {
  constructor($rootScope, $scope, $log, $http, $q, $timeout, $window, $mdDialog) {
    ...
    function sendRequest() {
      ...
      $http({
        ...
        try {
          ...
          if(data.sdf){
            $rootScope.$broadcast('update_sdf_sample', data.sample);
            self.showSmile = false;
           
          }else{
            self.showSmile = true;
          }
          console.log(self.showSmile)
      ...
      return new Promise(function (resolve, reject) {
        $http.get(REACTION_API).then(function (response) {
          ...
          self.reactionList.push({
            ...
            description: "",
            removeSalt: true,
            warn: "This option will take time to process",
            template: option_dir + "3dmaker.html"
          })
          self.reactionList.push({
            id: 1,
            display: "Preparator",
            ...
            addHs: true,
            removeSalt: true,
            template: option_dir + "preparator.html"
          })
          resolve()
        });
      })
